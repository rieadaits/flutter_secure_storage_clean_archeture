...
import '../../data/data_sources/service/users_service.dart';
...
Future<void> init() async {
  ...
  sl.registerLazySingleton<AuthenticationRemoteDataSources>(
    () => AuthenticationRemoteDataSourcesImpl(authenticationService: sl(), storage: sl()),
  );
  sl.registerLazySingleton<UserRemoteDataSource>(
    () => UserRemoteDataSourceImpl(client: sl(), service: sl()),
  );

    // API Service (Retrofit)
  sl.registerLazySingleton<AuthenticationService>(
    () => AuthenticationService(sl<DioClient>().dio),
  );
